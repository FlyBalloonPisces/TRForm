...
0xe, 0x9b, 0xdc, 0xa8, 0xdf, 0x64
        ...
        public static List<string> GetMapAttrs(string[] args)
        {
            ...
            for (int j = 0; j < args.Length; j++)
            {
                ...
                //int infoNum = 0;
                FileInfo fileInfo = new FileInfo(args[j]);
                ...
                while (num < file_num)
                {
                    ...
                    if (file_title.EndsWith("_info.txt"))
                    {
                        int part_num =
                        ...
                        {
                            if (!readAllLines[i].StartsWith("#") && !readAllLines[i].StartsWith("<") && readAllLines[i].Contains("="))
                            {
                                string attr = readAllLines[i].Split('=')[0].Trim().ToLower();
                                //string value = readAllLines[i].Split('=')[1].Trim();
                                if (!attrs.Contains(attr))
                                {
                                    attrs.Add(attr);
                                }

                                //if (!keyValuePairs.ContainsKey(attr))
                                //{
                ...
                //string[] attrsA = attrs.ToArray();
                //string[] valuesA = values.ToArray();

                //string[,] results = new string[attrsA.Length, values.Count / 2 + 1];
                //for (int i = 0; i < attrsA.Length; i++)
                //{
                //    results[i, 0] = attrsA[i];
                //}

                //for (int i = 0;i < valuesA.Length; i+= 2)
                //{
                //    int row = attrs.IndexOf(valuesA[i]);
        ...
        public static SortedList<int, string[]> GetMapAttrValues(string[] args, List<string> attrs)
        {
            int infoNum = 0;
            ...
            for (int j = 0; j < args.Length; j++)
            {
                ...
                while (num < file_num)
                {
                    ...
                    if (file_title.EndsWith("_info.txt"))
                    {
                        ...
                        //SortedDictionary<string, string> keyValuePairs = new SortedDictionary<string, string>();
                        string[] values = new string[attrs.Count];
                        for (int i = 0; i < readAllLines.Length; i++)
                        {
                            if (!readAllLines[i].StartsWith("#") && !readAllLines[i].StartsWith("<") && readAllLines[i].Contains("="))
                            {
                                string attr = readAllLines[i].Split('=')[0].Trim();
                                string value = readAllLines[i].Split('=')[1].Trim();

                                int index = attrs.IndexOf(attr.ToLower());
                                values[index] = value;
                            }
                        }
                        string attr2 = "pkgname";
                        string value2 = pkg_name;

                        int index2 = attrs.IndexOf(attr2.ToLower());
                        values[index2] = value2;

                        attr2 = "infoname";
                        value2 = file_title;

                        index2 = attrs.IndexOf(attr2.ToLower());
                        values[index2] = value2;

                        list.Add(infoNum, values);
                        infoNum++;
                    }
